...
class Printer
  ...
  def call
    ...
    puts Terminal::Table.new(title: title('result', 'light_white'), rows: rows,
  ...
  def statistic_data
    table = Terminal::Table.new(title: title('statistic', 'light_white'),
                                style: { width: search_size, border_bottom: false }) do |t|
      t.add_row [title('total', 'green'), statistic[:total_quantity].to_s.colorize(:light_blue)]
      t.add_row [title('requests', 'green'), statistic[:requests_quantity].to_s.colorize(:light_blue)]
    end
    puts table
  end

  def title(text, color)
    I18n.t("print.#{text}").colorize(:"#{color}")
  end
